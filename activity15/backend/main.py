...
Features:
  - Keyword-based vector store fallback for retrieval
...
class VectorStore:
    ...
    def _score(self, query_tokens: List[str], text_tokens: List[str]) -> float:
        ...
        if not common:
            return 0.0
        return float(len(common)) / float(len(set(text_tokens)))

    def add(self, col: str, texts: List[str], metas: List[dict]) -> None:
        if col not in self.db:
            self.db[col] = []
        for text, meta in zip(texts, metas):
            self.db[col].append({"text": text, "meta": meta, "tokens": self._tokenize(text)})
    ...
    def search(self, col: str, query: str, top_k: int = 5) -> List[dict]:
        ...
        for item in self.db[col]:
            score = self._score(query_tokens, item.get("tokens", []))
            if score > 0.0:
...
AMBIGUOUS_PHRASES = {
    'that thing', 'that stuff', 'this thing', 'something', 'anything',
    'whatever', 'it', 'this', 'that', 'the thing', 'the stuff'
}


def is_ambiguous_query(query: str) -> bool:
    q = query.lower().strip()
    if any(phrase in q for phrase in AMBIGUOUS_PHRASES):
        return True
    tokens = [w for w in re.findall(r'\w+', q) if w not in STOP_WORDS]
    if not tokens:
        return True

    # Short but valid definitions/questions are OK if they contain a noun/topic candidate.
    if len(q) < 20:
        topic_terms = [t for t in tokens if t not in {'what', 'is', 'are', 'was', 'were', 'do', 'does', 'can', 'could', 'will', 'would'}]
        if topic_terms:
            return False
        return True
...
Answer:"""
    ...
    if is_ambiguous_query(query):
        answer = (
            ...
            "Can you rephrase the question with the specific topic or object?"
        )
        transcript = [
            {"phase": "USER", "content": query},
            {"phase": "ACTION", "content": "Identify ambiguity in the user query."},
            {"phase": "OBSERVE", "content": "Query lacks a clear topic or contains vague referents."},
    ...
    transcript = [
        {"phase": "USER", "content": query},
        {"phase": "ACTION", "content": "Retrieve relevant transcript chunks and generate an answer."},
        {"phase": "OBSERVE", "content": context},
        {"phase": "ANSWER", "content": answer},
    ]
