...
Your goal is to take a high-level objective and break it into exactly 5 sequential steps.
...
def autonomous_planner(goal):
    ...
    while steps_taken < max_steps:
        ...
        response = client.models.generate_content(
            ...
            config={
                "system_instruction": planner_identity
            },
            contents=(
                f"Goal: {goal}. "
                f"Current steps completed: {plan}. "
                f"Analyze the goal and provide Step {steps_taken + 1}:"
            )
        )
...
if __name__ == "__main__":
    ...
    for i, step in enumerate(final_plan, start=1):
        print(f"STEP {i}: {step}")